...
function card(r, mobile) {
  ...
  // 26px is a comfortable mouse target and a poor thumb one — the WCAG floor is 44.
  // The bar is full-height either way, so on a phone it only has to grow sideways.
  const togBase = 'r-toggle p-0 border-0 cursor-pointer flex-none self-stretch -ml-[14px] rounded-l-[13px] disabled:cursor-default '
    + (mobile ? 'w-[44px]' : 'w-[26px]');
  ...
  el.innerHTML = `
    ...
    <div class="r-metric text-right flex-none flex flex-col items-end gap-1">
      <div class="r-temp cursor-pointer ${curColor} ${mobile ? 'text-[1.6rem]' : 'text-[2rem]'} font-extrabold leading-none tabular-nums">${temp}${temp === '—' ? '' : '<span class="text-[1.1rem] font-bold opacity-50 ml-px">°</span>'}</div>
