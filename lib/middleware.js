...
function cookies(req) {
  const out = {}; (req.headers.cookie || '').split(';').forEach((p) => {
    ...
    try { out[key] = decodeURIComponent(raw); } catch { out[key] = raw; } // #57 — malformed % doesn't crash
  });
  return out;
}
...
async function currentUser(req) {
  ...
  // Extend session expiry on activity — skip if >1h remains (#63, avoid write on every poll)
  if (s.exp - Date.now() < 60 * 60 * 1000) db.saveSession(t, s.user, Date.now() + SESSION_MS).catch(() => {});
  return s.user;
}

// gate all mutations — config changes AND relay toggling; viewing stays open (#79)
